"""红黑统计公报库地级市 2022—2025 年公报批量适配器。

红黑统计公报库是公开的统计公报转载索引。本适配器只接受标题明确指向
地级行政单元本身的 2022—2025 年公报，并把页面正文作为 B2 精确二手来源：
...
def _write_snapshot(snapshot_path: Path, items: Iterable[Mapping[str, Any]]) -> None:
    output = {
        ...
        "selection_note": "只保留标题明确为2022—2025年地级行政单元本身的统计公报；区县公报、图表估读和无法解析的页面排除。数值以页面正文直接读取，来源等级为B2。",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="抓取红黑统计公报库地级市2022—2025年公报")
